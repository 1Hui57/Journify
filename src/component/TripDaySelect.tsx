...
interface TripDaySelectProps {
    ...
    deleteTripDate: (deleteDayId: string) => void;
}
export default function TripDaySelect({ item, selectedDay, selectDate, deleteTripDate }: TripDaySelectProps) {
    ...
    return (
        <div onClick={() => selectDate(item.id, item.rawDate)}
            ...
            <p>第{item.number}天</p>
            {isDayHover &&
                <MdDelete onClick={(e)=>{e.stopPropagation();deleteTripDate(item.id)}}
                className="absolute w-4 h-4 top-1 right-1 text-primary-800" />
            }
